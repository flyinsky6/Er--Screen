...
This study proposes a complete computational prediction framework for ERα-targeted drug discovery, including ERα bioactivity regression, ADMET classification, and drug-target interaction prediction. Feature selection was implemented using R with caret and rfSA, while ensemble learning and dual-channel DNN modeling were performed in Python. SMOTE, Gaussian augmentation, and multi-step feature selection were applied to address data imbalance and redundancy. Finally, an integrated model combining XGBoost, LightGBM, and dual-channel DNN was established for full-flow prediction.

## Requirement
- R: caret, corrplot, doParallel
- Python:
